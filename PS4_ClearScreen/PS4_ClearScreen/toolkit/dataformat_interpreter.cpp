...
{
public:
	Gnm::SurfaceFormat m_format; ///< The <c>Gnm::SurfaceFormat</c> that this instance addresses.
	uint8_t m_bitsPerChannel[4]; ///< For each of four channels, the number of bits used to store the channel. Does not apply to block-compressed formats.
};

#define DEFINE_SURFACEFORMATINFO(S,X,Y,Z,W) \
{(S), {(X), (Y), (Z), (W)}}

static const SurfaceFormatInfo g_surfaceFormatInfo[Gnm::kSurfaceFormat1Reversed + 1] =
{
	/* 0x00 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormatInvalid    ,  0,  0,  0,  0),
	/* 0x01 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat8          ,  8,  0,  0,  0),
	/* 0x02 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat16         , 16,  0,  0,  0),
	/* 0x03 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat8_8        ,  8,  8,  0,  0),
	/* 0x04 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat32         , 32,  0,  0,  0),
	/* 0x05 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat16_16      , 16, 16,  0,  0),
	/* 0x06 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat10_11_11   , 11, 11, 10,  0),
	/* 0x07 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat11_11_10   , 10, 11, 11,  0),
	/* 0x08 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat10_10_10_2 ,  2, 10, 10, 10),
	/* 0x09 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat2_10_10_10 , 10, 10, 10,  2),
	/* 0x0a */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat8_8_8_8    ,  8,  8,  8,  8),
	/* 0x0b */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat32_32      , 32, 32,  0,  0),
	/* 0x0c */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat16_16_16_16, 16, 16, 16, 16),
	/* 0x0d */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat32_32_32   , 32, 32, 32,  0),
	/* 0x0e */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat32_32_32_32, 32, 32, 32, 32),
	/* 0x0f */ {},
	/* 0x10 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat5_6_5      ,  5,  6,  5,  0),
	/* 0x11 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat1_5_5_5    ,  5,  5,  5,  1),
	/* 0x12 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat5_5_5_1    ,  1,  5,  5,  5),
	/* 0x13 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat4_4_4_4    ,  4,  4,  4,  4),
	/* 0x14 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat8_24       , 24,  8,  0,  0),
	/* 0x15 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat24_8       ,  8, 24,  0,  0),
	/* 0x16 */ {},
	/* 0x17 */ {},
	/* 0x18 */ {},
	/* 0x19 */ {},
	...
	/* 0x39 */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat4_4        ,  4,  4,  0,  0),
	/* 0x3a */ DEFINE_SURFACEFORMATINFO(Gnm::kSurfaceFormat6_5_5      ,  5,  5,  6,  0),
	/* 0x3b */ {},
	/* 0x3c */ {},
};
...
{
	SCE_GNM_ASSERT(nullptr != pointer);
	SCE_GNM_ASSERT(count <= 64);
...
void insertBitfield(void *pointer, uint64_t offset, uint64_t count, uint64_t value)
{
	SCE_GNM_ASSERT(nullptr != pointer);
	SCE_GNM_ASSERT(count <= 64);
	void *p = static_cast<uint8_t *>(pointer) + (offset >> 3);
	offset &= 7;
	const uint64_t mask    = (uint64_t(1) << count) - 1;
	const uint64_t andMask = mask << offset;
	const uint64_t orMask  = value << offset;
	switch((offset + count + 7) / 8)
	{
	case 1:
		*static_cast<uint8_t *>(p) &= ~andMask;
		*static_cast<uint8_t *>(p) |= orMask;
		break;
	case 2:
		*static_cast<uint16_t *>(p) &= ~andMask;
		*static_cast<uint16_t *>(p) |= orMask;
		break;
	case 3:
	case 4:
		*static_cast<uint32_t *>(p) &= ~andMask;
		*static_cast<uint32_t *>(p) |= orMask;
		break;
	case 5:
	case 6:
	case 7:
	case 8:
		*static_cast<uint64_t *>(p) &= ~andMask;
		*static_cast<uint64_t *>(p) |= orMask;
		break;
	}
}

inline bool isSupported(Gnm::SurfaceFormat surfaceFormat)
{
	switch(surfaceFormat)
	{
	    case Gnm::kSurfaceFormat32:
	    case Gnm::kSurfaceFormat32_32:
	    case Gnm::kSurfaceFormat32_32_32_32:
        ...
        case Gnm::kSurfaceFormat5_9_9_9:
            return false;
        default:
            return true;
    }
...
Gnm::TextureChannelType getRealChannelType(Gnm::TextureChannelType purportedChannelType, int count)
{
	if(count >= 6)
	...
	{
	case Gnm::kTextureChannelTypeUBScaled:
	case Gnm::kTextureChannelTypeSScaled:
	case Gnm::kTextureChannelTypeUScaled:
		return Gnm::kTextureChannelTypeUScaled;
	case Gnm::kTextureChannelTypeUBInt:
	case Gnm::kTextureChannelTypeSInt:
	case Gnm::kTextureChannelTypeUInt:
		return Gnm::kTextureChannelTypeUInt;
	default:
		return Gnm::kTextureChannelTypeUNorm;
	}
}
...
void dataFormatDecoder(Reg32 *__restrict destination, const uint32_t *__restrict source, Gnm::DataFormat dataFormat)
{
	if(false == isSupported(dataFormat.getSurfaceFormat()))
		return;
	if(Gnm::kTextureChannelTypeSrgb == dataFormat.getTextureChannelType())
		switch(dataFormat.getTotalBytesPerElement())
		{
		case 1:
			dataFormat.m_bits.m_surfaceFormat = Gnm::kSurfaceFormat8;
		case 2:
			dataFormat.m_bits.m_surfaceFormat = Gnm::kSurfaceFormat8_8;
		default:
			dataFormat.m_bits.m_surfaceFormat = Gnm::kSurfaceFormat8_8_8_8;
			break;
		}
	const auto numComponents = (Gnm::kSurfaceFormat5_9_9_9 == dataFormat.getSurfaceFormat()) ? 3 : dataFormat.getNumComponents();
	const Gnm::TextureChannel textureChannel[4] =
	{
		static_cast<Gnm::TextureChannel>(dataFormat.m_bits.m_channelX),
		static_cast<Gnm::TextureChannel>(dataFormat.m_bits.m_channelY),
		static_cast<Gnm::TextureChannel>(dataFormat.m_bits.m_channelZ),
		static_cast<Gnm::TextureChannel>(dataFormat.m_bits.m_channelW),
	};
	const bool constantValuesAreInteger = Gnm::kTextureChannelTypeUInt == dataFormat.getTextureChannelType() || Gnm::kTextureChannelTypeSInt == dataFormat.getTextureChannelType();
	const uint32_t constantValue[2] = {0x00000000, constantValuesAreInteger ? 0x00000001U : 0x3f800000U};
	const auto &info = g_surfaceFormatInfo[dataFormat.getSurfaceFormat()];
	struct TexelChannel
	{
		uint32_t m_offset;
		uint32_t m_count;
		Gnm::TextureChannelType m_type;
	};
	...
	for(auto c = 0; c < 4; ++c)
	{
		...
		channel[c].m_type = dataFormat.getTextureChannelType();
        channel[c].m_type = getRealChannelType(channel[c].m_type, channel[c].m_count);
		bitOffset += channel[c].m_count;
	}
	uint32_t encoded[4] = {};
	for(auto c = 0U; c < dataFormat.getNumComponents(); ++c)
		encoded[c] = static_cast<uint32_t>(extractBitfield(source, channel[c].m_offset, channel[c].m_count));
	if(Gnm::kSurfaceFormat5_9_9_9 == dataFormat.getSurfaceFormat())
		for(auto c = 0; c < 3; ++c)
			encoded[c] |= encoded[3] << 9;
	Channel decoded[4] = {};
	for(auto c = 0U; c < numComponents; ++c)
	{
		const uint32_t bitCount = (Gnm::kSurfaceFormat5_9_9_9 == dataFormat.getSurfaceFormat()) ? 14 : channel[c].m_count;
		...
		case Gnm::kTextureChannelTypeUScaled:
			...
			break;
		case Gnm::kTextureChannelTypeUBNorm:
			u.i += unsignedToSigned;
			u.i &= mask;
		case Gnm::kTextureChannelTypeSNorm:
            decoded[c].f = std::max((float)signExtend(u.u, bitCount) / ((1 << (bitCount-1)) - 1), -1.0f);
			break;
		case Gnm::kTextureChannelTypeUBNormNoZero:
			u.i += unsignedToSigned;
			u.i &= mask;
		case Gnm::kTextureChannelTypeSNormNoZero:
		...
		case Gnm::kTextureChannelTypeSrgb:
            ...
            else
            {
                ...
                const auto clamped = std::min(255U, unclamped);
                decoded[c].u = lookup_8_srgb[clamped];
            }
			break;
        case Gnm::kTextureChannelTypeFloat:
            if (bitCount == 32)
                decoded[c].f = u.f;
            else
                decoded[c].f = Gnmx::Toolkit::unpackFloat(u.u, floats[bitCount].m_signBits, floats[bitCount].m_exponentBits, floats[bitCount].m_mantissaBits);
			break;
		}
	}
	for(auto c = 0; c < 4; ++c)
		...
		{
		case Gnm::kTextureChannelConstant0:
			destination[c].u = constantValue[0];
			break;
		case Gnm::kTextureChannelConstant1:
			destination[c].u = constantValue[1];
			break;
		case Gnm::kTextureChannelX:
			destination[c].u = decoded[0].u;
			break;
		case Gnm::kTextureChannelY:
			destination[c].u = decoded[1].u;
			break;
		case Gnm::kTextureChannelZ:
			destination[c].u = decoded[2].u;
			break;
		case Gnm::kTextureChannelW:
...
void dataFormatEncoder(uint32_t *__restrict dest, uint32_t *__restrict destDwords, const Reg32 *__restrict src, const Gnm::DataFormat dataFormat)
{
    ...
    enum {kNoSourceChannel = -1};
    int fromSourceChannel[4] = {kNoSourceChannel, kNoSourceChannel, kNoSourceChannel, kNoSourceChannel};
	for(auto c = 3; c >= 0; --c)
        if(textureChannel[c] >= Gnm::kTextureChannelX)
            fromSourceChannel[textureChannel[c] - Gnm::kTextureChannelX] = c;
    ...
    {
        Reg32 encoded = {};
        const auto count = info.m_bitsPerChannel[c];
        if(fromSourceChannel[c] != kNoSourceChannel)
        {
		    Gnm::TextureChannelType type = dataFormat.getTextureChannelType();
            type = getRealChannelType(type, count);
            if(3 == c && Gnm::kTextureChannelTypeSrgb == type)
                type = Gnm::kTextureChannelTypeUNorm;
            Reg32 decoded = src[fromSourceChannel[c]];
            switch(type)
            {
                case Gnm::kTextureChannelTypeUNorm:
			    case Gnm::kTextureChannelTypeSrgb:
                    decoded.f = std::max(0.f, std::min(1.f, decoded.f));
                    break;
			    case Gnm::kTextureChannelTypeSNorm:
			    case Gnm::kTextureChannelTypeUBNorm:
			    case Gnm::kTextureChannelTypeSNormNoZero:
			    case Gnm::kTextureChannelTypeUBNormNoZero:
                    decoded.f = std::max(-1.f, std::min(1.f, decoded.f));
                    break;
                default:
                    break;
            }
            const uint32_t maxu = (1ULL << count) - 1;
            const int32_t maxi = maxu >> 1;
            const int32_t mini = -(maxi + 1);
            switch(type)
            {
			    case Gnm::kTextureChannelTypeSrgb:
                    decoded.f = powf(decoded.f, 0.45f);
                case Gnm::kTextureChannelTypeUNorm:
                    encoded.u = static_cast<uint32_t>(std::round(decoded.f * maxu));
                    break;
			    case Gnm::kTextureChannelTypeSNorm:
			    case Gnm::kTextureChannelTypeUBNorm:
                    encoded.i = static_cast<int32_t>(std::round(decoded.f * maxi));
                    break;
			    case Gnm::kTextureChannelTypeUScaled:
                    encoded.u = static_cast<uint32_t>(std::round(decoded.f));
                    break;
			    case Gnm::kTextureChannelTypeSScaled:
			    case Gnm::kTextureChannelTypeUBScaled:
                    encoded.i = static_cast<int32_t>(std::round(decoded.f));
                    break;
			    case Gnm::kTextureChannelTypeUInt:
                    encoded.u = decoded.u;
                    break;
			    ...
			    case Gnm::kTextureChannelTypeSNormNoZero:
			    case Gnm::kTextureChannelTypeUBNormNoZero:
                    encoded.i = static_cast<int32_t>(std::round((decoded.f * 0.5f + 0.5f) * maxu)) + mini;
                    break;
			    case Gnm::kTextureChannelTypeFloat:
                    if(count == 32)
                        encoded.f = decoded.f;
                    else
                        encoded.u = packFloat(decoded.f, floats[count].m_signBits, floats[count].m_exponentBits, floats[count].m_mantissaBits);
                    break;
            }
            switch(type)
            {
            ...
            switch(type)
            {
			    ...
			    case Gnm::kTextureChannelTypeUBScaled:
			    case Gnm::kTextureChannelTypeUBInt:
                    encoded.u = std::min(maxu, encoded.u);
                    break;
			    case Gnm::kTextureChannelTypeSScaled:
			    case Gnm::kTextureChannelTypeSInt:
                    encoded.i = std::max(mini, std::min(maxi, encoded.i));
                    break;
                default:
                    break;
            }
        }
        insertBitfield(dest, offset, count, encoded.u);
        offset += count;
    }
    const auto bytesPerElement = dataFormat.getTotalBytesPerElement();
    switch(bytesPerElement)
    {
        case 1:
            ((uint8_t *)dest)[1] = ((uint8_t *)dest)[0];
        case 2:
            ((uint16_t *)dest)[1] = ((uint16_t *)dest)[0];
        default:
            break;
    }
    *destDwords = (bytesPerElement + sizeof(uint32_t) - 1) / sizeof(uint32_t);
}
